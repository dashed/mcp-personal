...
async def test_search_files_finds_python(tmp_path: Path, client):
    ...
    # Act
    from mcp.types import ToolRequest, ClientRequest
...
async def test_search_files_with_flags(tmp_path: Path, client):
    ...
    result_no_hidden = await client.send_request(
        ClientRequest(
            ToolRequest(
                method="tools/call",
                params={
    ...
    result_with_hidden = await client.send_request(
        ClientRequest(
            ToolRequest(
                method="tools/call",
                params={
                    "name": "search_files",
                    "arguments": {"pattern": r"\.py$", "path": str(tmp_path), "flags": "--hidden"}
                }
            )
        )
    )
...
async def test_search_files_error_handling(client):
...
async def test_filter_files_returns_best_match(tmp_path: Path, client):
...
async def test_filter_files_multiple_matches(tmp_path: Path, client):
...
async def test_filter_files_error_handling(client):
...
async def test_list_tools(client):
    ...
    from mcp.types import ListToolsRequest, ClientRequest
...
async def test_search_files_mocked(mock_check_output, client):
        ...
        result = await client.send_request(
            ClientRequest(
                ToolRequest(
                    ...
                    params={
                        "name": "search_files",
                        "arguments": {"pattern": r"\.py$", "path": "src"}
                    }
                )
            )
        )
...
@patch("subprocess.check_output")
async def test_filter_files_mocked(mock_check_output, mock_popen, client):
        ...
        result = await client.send_request(
            ClientRequest(
                ToolRequest(
                    ...
                    params={
                        "name": "filter_files",
                        "arguments": {"filter": "main", "pattern": r"\.py$"}
                    }
                )
            )
        )
